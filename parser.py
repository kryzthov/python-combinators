...
class Input(object):
    ...
    def NextChar(self):
        ...
        return Input(
            text=self._text, pos=self._pos + 1, line=line, column=column)
...
class Regex(ParserBase):
    ...
    def Parse(self, input):
        ...
        if match is None:
            ...
            logging.log(LogLevel.DEBUG_VERBOSE, 'Matched Regex(%r) = %r',
                        self._pattern.pattern, matched_str)
            return Success(
                match=matched_str,
                value=matched_str,
...
class Ref(ParserBase):
    ...
    def Parse(self, input):
        assert (self._ref is not None), ('Unbound parser reference: %r.' %
                                         (self, ))
...
class Integer(ParserBase):
    ...
    def __init__(self, base=10, prefix=''):
        ...
        self._base = base
        self._digit_parser = Regex(r'-?%s%s+' % (prefix,
                                                 GetRangeForBase(base)))
